...
import { X, Upload, Loader2, Plus, Trash2, Copy, Check, Type, AlignLeft, Tags, Palette, Ruler, Image as ImageIcon, Settings, Award, ChevronDown, ChevronRight, FileText, LayoutList } from 'lucide-react';
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-xs text-gray-400">{countWords(formData.description)} words</span>
                </div>
                <textarea
                  name="description"
                  ...
                  onChange={handleChange}
                  rows={5}
                  className={`w-full px-4 py-2.5 border rounded-xl text-sm focus:ring-2 focus:ring-orange-200 focus:border-orange-400 transition-all resize-y ${errors.description ? 'border-red-400 bg-red-50' : 'border-gray-300'}`}
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-xs text-gray-400">{countWords(formData.shortDescription)} words</span>
                </div>
                <textarea
                  ...
                  placeholder={"e.g.,\n- Same Day Delivery available on select Pin Codes\n- Order before 12 noon for same day dispatch\n- Cash on Delivery available for Standard delivery"}
                />
                <p className="text-xs text-gray-400 mt-1">Key points shown in the hero section. Use new lines for bullet points.</p>
              </div>
            </div>
          </div>
                ...
                {formData.sections.map((section, index) => (
                  <div key={index} className="border border-gray-200 rounded-xl p-4 bg-gray-50/50">
                    ...
                    <textarea
                      ...
                      rows={4}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-200 focus:border-orange-400 resize-y"
                      placeholder="Section content... Use new lines for paragraphs or bullet points."
                    />
                    <p className="text-xs text-gray-400 mt-1">{countWords(section.content)} words</p>
                  </div>
                ))}
              </div>
            )}
